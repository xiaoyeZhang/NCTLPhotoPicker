...
public struct TLPhotosPickerConfigure {
    ...
    public var selectAll = "Select All"
    public var deselectAll = "Deselect All"
    public var cancelIcon: UIImage? = nil
    public var emptyImage: UIImage? = nil
    public var selectAllButton = false
    ...
    public var navigationBgColor: UIColor? = nil
    public var viewBgColor: UIColor? = nil
    public var textColor: UIColor? = nil // 文字颜色
    public var selectedColor = UIColor(red: 88/255, green: 144/255, blue: 255/255, alpha: 1.0)
    public var customViewBtn: UIButton? = nil //修改  新增右侧导航栏按钮自定义视图
...
open class TLPhotosPickerViewController: UIViewController {
    ...
    @objc func configureNavBarForViewController() {
        
        if self.configure.navigationBgColor != nil {
            if #available(iOS 13.0, *) {
                let appearance = UINavigationBarAppearance()
                appearance.backgroundColor = self.configure.navigationBgColor // 设置导航栏背景色
                navigationBar.scrollEdgeAppearance = appearance// 带scroll滑动的页面
                navigationBar.standardAppearance = appearance// 常规页面
            } else {
                navigationBar.isTranslucent = false
                navigationBar.barTintColor = self.configure.navigationBgColor
                navigationBar.backgroundColor = self.configure.navigationBgColor
            }
        }
    }
    ...
    private func updateUserInterfaceStyle() {
        if #available(iOS 13.0, *) {
            ...
            if userInterfaceStyle.rawValue == 2 {
                ...
                self.view.backgroundColor = self.configure.navigationBgColor != nil ? self.configure.navigationBgColor : .black
                self.collectionView.backgroundColor = self.configure.navigationBgColor != nil ? self.configure.navigationBgColor : .black
            } else {
                self.popArrowImageView.image = image?.colorMask(color: .white)
                self.subTitleArrowImageView.image = subImage
                self.view.backgroundColor = self.configure.navigationBgColor != nil ? self.configure.navigationBgColor : .white
                self.collectionView.backgroundColor = self.configure.navigationBgColor != nil ? self.configure.navigationBgColor : .white
            }
        }
        if self.configure.textColor != nil {
            self.photosButton.tintColor = self.configure.textColor
        }
        configureNavBarForViewController()
    }
...
//        self.subTitleLabel.text = self.configure.tapHereToChange
        self.subTitleLabel.text = self.configure.customLocalizedTitle["Camera Roll"]
        self.cancelButton.title = self.configure.cancelTitle
        if self.configure.cancelIcon != nil {
            self.cancelButton.title = ""
            if self.configure.textColor != nil {
                self.cancelButton = UIBarButtonItem(image: self.configure.cancelIcon?.imageColor(self.configure.textColor!), style: .plain, target: self, action:#selector(closeButton(sender:)))
            } else {
                self.cancelButton = UIBarButtonItem(image: self.configure.cancelIcon?.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action:#selector(closeButton(sender:)))
            }
        }
        if self.configure.selectAllButton {
            self.selectAllButton.title = self.configure.selectAll
            self.customNavItem.leftBarButtonItems = [self.cancelButton, self.selectAllButton]
        }
        ...
        if let customView = self.configure.customViewBtn {
            customView.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            customView.frame = CGRect(x: customView.frame.origin.x, y: customView.frame.origin.y, width: labelWidth(self.configure.doneTitle, UIFont.systemFont(ofSize: 14)), height: 26)
            customView.setTitleColor(.white, for: .normal)
            customView.setTitle(self.configure.doneTitle, for: .normal)
            customView.addTarget(self, action: #selector(doneCustomViewButtonTap(_:)), for: .touchUpInside)
            customView.backgroundColor = self.configure.selectedColor
            customView.layer.cornerRadius = 15 // 设置圆角半径
            customView.clipsToBounds = true
            // 创建 UIBarButtonItem，并将自定义视图添加进去
            doneButton.customView = self.configure.customViewBtn
        }
    ...
    // 计算文本长度
    func labelWidth(_ text: String, _ font: UIFont = UIFont.systemFont(ofSize: 14)) -> CGFloat {
        let attributes = [NSAttributedString.Key.font: font]
        let textSize = (text as NSString).size(withAttributes: attributes)
        return textSize.width + 40 // 增加一些边距
    }
...
//        self.titleLabel.text = self.focusedCollection?.title
        self.subTitleLabel.text = self.focusedCollection?.title
    ...
    @objc func doneCustomViewButtonTap(_ sender:Any) {
        doneButtonTap()
    }
    ...
    /* //MARK: 修改第三方视图 */
    @IBAction open func selectAllButtonTap(sender: UIBarButtonItem) {
        guard let collection = self.focusedCollection else { return }

        if collection.count == 0 || collection.sections == nil {
            return
        }
        
        let titleName = self.selectAllButton.title
        let isSelectAll: Bool = (titleName == self.configure.selectAll) ? true : false
        self.selectAllButton.title = isSelectAll ? self.configure.deselectAll : self.configure.selectAll
        
        self.indicator.startAnimating()
        
        var indexPaths: [IndexPath] = []
        if self.customDataSouces != nil {
            for section in 0 ..< collection.sections!.count {
                for index in 0 ..< (collection.sections?[safe: section]?.assets.count)! {
                    let indexPath = IndexPath(row: index, section: section)
                    let isCameraRow = collection.useCameraButton && section == 0 && index == 0
                    if !isCameraRow {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
            if let strongSelf = self {
                for index in 0 ..< collection.count {
                    autoreleasepool{
                        if strongSelf.customDataSouces == nil {
                            let indexPath = IndexPath(row: index, section: 0)
                            let isCameraRow = collection.useCameraButton && index == 0
                            if !isCameraRow {
                                indexPaths.append(indexPath)
                            }
                        }
                        if isSelectAll {
                            guard let result = collection.fetchResult, index < result.count else { return }
                            var tlAsset = TLPHAsset(asset: result.object(at: max(index,0)))
//                            var tlAsset = TLPHAsset(asset: collection.fetchResult![index])
                            if !strongSelf.selectedAssets.contains(where: { $0.phAsset == tlAsset.phAsset }) {
                                tlAsset.selectedOrder = strongSelf.selectedAssets.count + 1
                                strongSelf.selectedAssets.append(tlAsset)
                            }
                        }
                    }
                }
                ...
                if strongSelf.customDataSouces != nil {
                    for section in 0 ..< collection.sections!.count {
                        autoreleasepool{
                            ...
                            let isCameraRow = collection.useCameraButton && section == 0
                            if isCameraRow { return }
                            if let reuseView = strongSelf.collectionView.supplementaryView(forElementKind: UICollectionView.elementKindSectionHeader, at: indexPath) {
                                if let section = strongSelf.focusedCollection?.sections?[safe: indexPath.section] {
                                    strongSelf.customDataSouces?.changeExitAssets(supplement: reuseView, assets: section.assets, selectedAssets: strongSelf.selectedAssets)
                                }
                            }
                        }
                    }
...
//                cell.orderLabel?.text = "\(selectedAsset.selectedOrder)"
                ...
                if (self.customDataSouces != nil) {
                    ...
                    cell.orderIcon?.image = TLBundle.podBundleImage(named: "check")
                } else {
                    cell.orderLabel?.text = "\(selectedAsset.selectedOrder)"
                }
                /* --------------------- */
                ...
                cell.orderIcon?.image = TLBundle.podBundleImage(named: "disCheck")
            }
        }
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        //MARK: 修改第三方视图，刷新section数据
        let indexPath = IndexPath(row: 0, section: indexPath.section)
        if let reuseView = collectionView.supplementaryView(forElementKind: UICollectionView.elementKindSectionHeader, at: indexPath) {
            if let section = self.focusedCollection?.sections?[safe: indexPath.section] {
                self.customDataSouces?.changeExitAssets(supplement: reuseView, assets: section.assets, selectedAssets: selectedAssets)
            }
        }
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.orderIcon?.image = TLBundle.podBundleImage(named: "disCheck") //MARK: 修改第三方视图
        if let selectedAsset = getSelectedAssets(asset) {
            cell.selectedAsset = true
//            cell.orderLabel?.text = "\(selectedAsset.selectedOrder)"
            /* //MARK: 修改第三方视图 */
...
extension TLPhotosPickerViewController: UICollectionViewDelegateFlowLayout {
    open func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let reuseView = collectionView.dequeueReusableSupplementaryView(ofKind: kind,
                                                                        ...
                                                                        for: indexPath)
        if self.configure.viewBgColor != nil {
            reuseView.backgroundColor = self.configure.viewBgColor
        }
        if let section = self.focusedCollection?.sections?[safe: indexPath.section] {
//            self.customDataSouces?.configure(supplement: reuseView, section: section)
            //    MARK: 修改第三方视图，传递 IndexPath数据
            self.customDataSouces?.configure(supplement: reuseView, section: section, indexPath: indexPath, selectedAssets: selectedAssets)
        }
        return reuseView
    }
...
extension TLPhotosPickerViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if #available(iOS 13.0, *) {
            cell.contentView.backgroundColor = self.configure.viewBgColor != nil ? self.configure.viewBgColor : .systemBackground
            cell.backgroundColor = self.configure.viewBgColor != nil ? self.configure.viewBgColor : .systemBackground
        } else {
            if self.configure.viewBgColor != nil {
                cell.contentView.backgroundColor = self.configure.viewBgColor
                cell.backgroundColor = self.configure.viewBgColor
            }
        }
...
//            cell.orderLabel?.text = "\(asset.selectedOrder)"
    ...
    public func uploadDoneTitle() {
        /* //MARK: 修改第三方视图 */
        let selectedNum = selectedAssets.count == 0 ? "" : "(" + String(selectedAssets.count) + ")"
        self.doneButton.title = self.configure.doneTitle + selectedNum
        if let customView = self.configure.customViewBtn {
            customView.setTitle(self.configure.doneTitle + selectedNum, for: .normal)
            customView.frame = CGRect(x: customView.frame.origin.x, y: customView.frame.origin.y, width: labelWidth(self.configure.doneTitle + selectedNum, UIFont.systemFont(ofSize: 14)), height: 26)
        }
        guard let collection = self.focusedCollection else { return }

        if self.configure.selectAllButton {
            if selectedAssets.count == Int(collection.fetchResult!.count) {
                self.selectAllButton.title = self.configure.deselectAll
            } else {
                self.selectAllButton.title = self.configure.selectAll
            }
        }
